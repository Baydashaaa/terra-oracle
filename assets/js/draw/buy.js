...
async function buyTicketsKeplr() {
  ...
  try {
    ...
    if (btn) { btn.innerHTML = `Mint ${ticketCount > 1 ? ticketCount + ' NFTs' : 'NFT'} - ${drawFmt(ticketCount*pricePerTicket)} LUNC`; btn.disabled = false; }

    await loadAllData();

  } catch(e) {
    if (statusEl) statusEl.style.display = 'none';
    if (btn) { btn.disabled = false; btn.innerHTML = `Mint ${ticketCount > 1 ? ticketCount + ' NFTs' : 'NFT'} - ${drawFmt(ticketCount*LUNC_PER_TICKET)} LUNC`; }
