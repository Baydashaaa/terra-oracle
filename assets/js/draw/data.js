...
function updatePodiumPrizes() {
  ...
  if (totalEl) totalEl.textContent = drawFmt(pool) + ' LUNC';
  ...
  if (label) label.textContent = drawFmt(pool) + ' / ' + drawFmt(WEEKLY_MIN) + ' LUNC';
  ...
  if (WEEKLY_MIN <= 0) {
    ...
    if (label)   label.textContent = drawFmt(pool) + ' LUNC';
    ...
    if (pool >= WEEKLY_MIN) {
      ...
      const remaining = drawFmt(WEEKLY_MIN - pool);
