...
function updatePoolDisplay() {
  ...
  const _pl=document.getElementById('pool-lunc');if(_pl)_pl.textContent = drawFmt(poolPrize) + ' LUNC';
  const _pu=document.getElementById('pool-usd');if(_pu)_pu.textContent = luncPrice > 0 ? '≈ $' + poolUsd.toFixed(2) + ' USD' : '';

  // Seeded next round
  const _seed = document.getElementById('stat-seeded');if(_seed)_seed.textContent = drawFmt(seededLunc);
  ...
  const _sb=document.getElementById('stat-burned');if(_sb)_sb.textContent = drawFmt(Math.round(seededLunc)) + ' LUNC';
  ...
  if (currentLottery === 'weekly') {
    ...
    if (p1) p1.textContent = drawFmt(Math.floor(pool80 * 0.60)) + ' LUNC';
    if (p2) p2.textContent = drawFmt(Math.floor(pool80 * 0.25)) + ' LUNC';
    if (p3) p3.textContent = drawFmt(Math.floor(pool80 * 0.15)) + ' LUNC';
  }
...
function switchLottery(type) {
  ...
  if (isDaily) {
    ...
    if (last && addrEl) {
      ...
      if (prizeEl) prizeEl.textContent = drawFmt(last.prize) + ' LUNC';
  ...
  if (!isDaily) {
    ...
    if (p1El) p1El.textContent = drawFmt(Math.floor(prize80 * 0.60)) + ' LUNC';
    if (p2El) p2El.textContent = drawFmt(Math.floor(prize80 * 0.25)) + ' LUNC';
    if (p3El) p3El.textContent = drawFmt(Math.floor(prize80 * 0.15)) + ' LUNC';
    if (totalEl) totalEl.textContent = drawFmt(window._weeklyPoolBalance || pool) + ' LUNC';
