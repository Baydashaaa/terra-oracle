...
const DRAW_ADMIN_WALLET = 'terra15jt5a9ycsey4hd6nlqgqxccl9aprkmg2mxmfc6';
...
function updateWheelTickets() {
  ...
  if (poolEl) poolEl.textContent = drawFmt(realPool * PRIZE_SHARE) + ' ' + currency;
...
function showWinnerCard(data) {
  ...
  if (p) p.textContent = (data.prize ? drawFmt(data.prize) + ' LUNC' : '-') +
...
window.OracleDrawUI = {
  ...
  fmt:            function(v) { return drawFmt(v); },
