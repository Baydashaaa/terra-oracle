...
// The cards carry a radio each, so the browser tracks the choice correctly, but
// a 14px radio dot is the only thing that moves when you pick one — on a panel
// this large the selection is easy to miss.
//
// Styling happens here rather than in the HTML because the accent colour is
// read from the card's own text: whatever hue a tier is written in, its
// selected state matches, and retinting a tier never means editing two places.
const TIER_ACCENT_FALLBACK = { basic: '#7b5cff', priority: '#f0b429' };

function tierCardOf(input) {
  return input.closest('label') || input.closest('.tier-card') || input.parentElement;
}

// The brightest coloured text in the card, which is the tier name. Falls back to
// a constant if nothing in there is coloured.
function tierAccentOf(card, key) {
  const fallback = TIER_ACCENT_FALLBACK[key] || '#7b5cff';
  if (!card || typeof getComputedStyle !== 'function') return fallback;
  const muted = /rgba?\(\s*(\d+)[,\s]+(\d+)[,\s]+(\d+)/;
  for (const el of card.querySelectorAll('b, strong, span, div')) {
    const c = getComputedStyle(el).color;
    const m = muted.exec(c);
    if (!m) continue;
    const [r, g, b] = [+m[1], +m[2], +m[3]];
    // Skip greys and near-whites: body copy, not the tier name.
    const spread = Math.max(r, g, b) - Math.min(r, g, b);
    if (spread > 40 && !(r > 220 && g > 220 && b > 220)) return c;
  }
  return fallback;
}

function paintTierCards() {
  const inputs = document.querySelectorAll('input[name="question-tier"]');
  if (!inputs.length) return;

  inputs.forEach(input => {
    const card = tierCardOf(input);
    if (!card) return;
    const accent = tierAccentOf(card, (input.value || '').toLowerCase());
    const on = input.checked;

    card.style.transition  = 'border-color .18s ease, box-shadow .18s ease, background-color .18s ease, transform .18s ease';
    card.style.borderColor = on ? accent : 'rgba(84,147,247,0.18)';
    card.style.boxShadow   = on ? `0 0 0 1px ${accent}, 0 6px 22px -8px ${accent}` : 'none';
    card.style.background  = on ? `color-mix(in srgb, ${accent} 10%, transparent)` : '';
    card.style.transform   = on ? 'translateY(-2px)' : 'translateY(0)';
    card.style.cursor      = 'pointer';

    if (!card.dataset.tierHover) {
      card.dataset.tierHover = '1';
      card.addEventListener('mouseenter', () => {
        if (!input.checked) card.style.borderColor = `color-mix(in srgb, ${accent} 55%, transparent)`;
      });
      card.addEventListener('mouseleave', () => {
        if (!input.checked) card.style.borderColor = 'rgba(84,147,247,0.18)';
      });
    }
  });
}
...
function initTierCards() {
  ...
  if (!inputs.length) return;
  inputs.forEach(input => {
    if (input.dataset.tierBound) return;
    input.dataset.tierBound = '1';
    input.addEventListener('change', () => {
      paintTierCards();
      // The price panel is per-tariff, so switching tiers has to redraw it or the
      // button would keep quoting the old total.
      if (typeof updateVerifyBtnPrice === 'function' && connectedAddress) {
        try { updateVerifyBtnPrice(connectedAddress); } catch (e) {}
      }
    });
  });
  paintTierCards();
...
// The ask panel can be built after load, so watch for the radios appearing.
if (typeof document !== 'undefined') {
  document.addEventListener('DOMContentLoaded', initTierCards);
  if (typeof MutationObserver === 'function') {
    new MutationObserver(() => initTierCards())
      .observe(document.documentElement, { childList: true, subtree: true });
  }
}
window.initTierCards = initTierCards;
